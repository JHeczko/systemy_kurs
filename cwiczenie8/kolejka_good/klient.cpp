...
//SETUP 
    ...
    mqd_t desK = open_mq(nazwa.c_str(), O_CREAT | O_RDONLY | O_EXCL, 0644, &attrK );
    if(desK == -1){
        perror("mq_open failure K");
        exit(EXIT_FAILURE);
    }

    mqd_t desS = open_mq(SERWER_QUEUE, O_WRONLY);
    if(desS == -1){
...
//WCZYTYWANIE Z KLAWIATURY
    ...
    do{
            ...
            out = pids +'|'+ input + '|';
            cout << out << endl;
            if(send_mq(desS, out.data(), 20, 1) == -1){
            perror("mqsend K error");
            }
            help = 0;
            out.erase();
            if(receive_mq(desK, buf, 20, NULL) == -1){
