...
void exitf(){
    while(unlink_mq(SERWER_QUEUE) != -1);
...
//SETUP
    ...
    mqd_t desS = open_mq(SERWER_QUEUE, O_CREAT | O_RDONLY | O_EXCL, 0644, &attrS);
    if(desS == -1){
...
while(1){
    if(receive_mq(desS, buf, 20, NULL) == -1){
    ...
    #endif
    desK = open_mq(nazwa.data(), O_WRONLY) ;
    if(desK == -1){
        perror("mq_open failure S-side");
        exit(EXIT_FAILURE);
    }
    if(send_mq(desK, wynikS.data(), 20, 1) == -1){
